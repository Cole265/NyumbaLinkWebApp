...
<head>
    ...
    <title>Edit Property - Khomolanu</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
    <script defer src="https://unpkg.com/alpinejs@3.x.x/dist/cdn.min.js"></script>
</head>
<body class="bg-gray-50" x-data="editPropertyForm()" x-init="init()">
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="mb-8">
            ...
            <p class="text-lg text-gray-600">Update the details for your listing</p>
        </div>
        ...
        <form @submit.prevent="submitForm($event)" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-lg p-8">
            ...
            <div x-show="currentStep === 4" x-cloak>
                <h2 class="text-2xl font-bold text-gray-900 mb-6">Images & Status</h2>

                <div class="mb-6">
                    <p class="text-sm text-gray-600 mb-2">Existing images are kept. You can optionally upload additional images.</p>
                    <label class="block text-sm font-semibold text-gray-700 mb-3">Upload New Images (optional)</label>
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-blue-500 transition">
                        <input type="file" name="new_images[]" multiple accept="image/*" @change="handleFileSelect" 
                        ...
                        <label for="imageUpload" class="cursor-pointer">
                            ...
                            <p class="text-xs text-gray-400 mt-2">PNG, JPG up to 5MB each (Max 10 images)</p>
                        </label>
                    </div>

                    <div x-show="selectedFiles.length > 0" class="mt-4">
                        <p class="text-sm font-semibold text-gray-700 mb-2">New Images (<span x-text="selectedFiles.length"></span>)</p>
                        <div class="grid grid-cols-3 gap-4">
    ...
    <script>
        function editPropertyForm() {
            return {
                ...
                async init() {
                    const parts = window.location.pathname.split('/').filter(Boolean); // ['landlord','properties','{id}','edit']
                    this.propertyId = parts[2];
                ...
                async loadProperty() {
                    ...
                    try {
                        const response = await fetch(`/api/v1/landlord/properties/${this.propertyId}`, {
                            headers: {
                        ...
                        if (response.ok && data.success && data.data) {
                            const p = data.data;
                            this.formData.property_type = p.property_type || 'residential';
                            this.formData.title = p.title || '';
                            this.formData.description = p.description || '';
                            this.formData.price = p.price || '';
                            this.formData.currency = p.currency || 'MWK';
                            this.formData.bedrooms = p.bedrooms ?? '';
                            this.formData.bathrooms = p.bathrooms ?? '';
                            this.formData.size_sqm = p.size_sqm ?? '';
                            this.formData.is_furnished = !!p.is_furnished;
                            this.formData.city = p.city || '';
                            this.formData.district = p.district || '';
                            this.formData.area = p.area ?? '';
                            this.formData.latitude = p.latitude ?? '';
                            this.formData.longitude = p.longitude ?? '';
                            this.formData.status = p.status || 'draft';
                        } else {
                            alert(data.message || 'Failed to load property details.');
                            window.location.href = '/landlord/dashboard';
                        }
                    } catch (error) {
                        console.error('Error loading property:', error);
                        alert('An error occurred while loading the property.');
                        window.location.href = '/landlord/dashboard';
                    }
                },
                ...
                async submitForm(event) {
                    console.log('🏠 Starting property form update');
                    
                    const token = localStorage.getItem('auth_token');
                    if (!token) {
                        alert('Please login as a landlord to update a property.');
                        window.location.href = '/login';
                        return;
                    }
                    ...
                    try {
                        console.log('📤 Sending request to /api/v1/landlord/properties/' + this.propertyId);
                        const response = await fetch(`/api/v1/landlord/properties/${this.propertyId}`, {
                            method: 'POST',
                            headers: {
                                'Authorization': `Bearer ${token}`,
                        ...
                        if (response.ok && data.success) {
                            ...
                            const message = data.message 
                                ...
                                || `Failed to update property (status ${response.status}). Please check your details and try again.`;
